...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // MISSING:
  res = fwrite(reinterpret_cast<char*> (kv->value), slen_ho, 1, f);
  if (res != 1) {
      return 0;
  }
  total += slen_ho;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // MISSING:
    position.position = (intptr_t) payload;
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  // MISSING:
  header.toDiskFormat();
  size_t res = fwrite(&header, sizeof(header), 1, f);
  if (res != 1) {
      return 0;
  }
  total += sizeof(header);
  ...
  // MISSING:
  if (fseek(f, offset + 6, SEEK_SET) != 0) {
      return 0;
  }
  if (fwrite(wds->word, wordlen_ho, 1, f) != 1) {
      return 0;
  }
  // if (res != 1) {
  //     return 0;
...
static HWSize_t WriteHeader(FILE *f,
  ...
  for (HWSize_t i = 0; i < cslen; i++) {
      res = fread(reinterpret_cast<void*> (&charread), sizeof(char), 1, f);
      if (res != 1) {
          return 0;
      }

      crcobj.FoldByteIntoCRC(charread);
  }

  uint32_t checksum = crcobj.GetFinalCRC();
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // First, write the element position in
      element_position_rec elpos = {nextelpos};
      elpos.toDiskFormat();

      // then seek! don't forget this
      if (fseek(f, offset, SEEK_SET) != 0) {
          LLIteratorFree(it);
          return 0;
      }
      res = fwrite(&elpos, sizeof(elpos), 1, f);
      if (res != 1) {
          LLIteratorFree(it);
          return 0;
      }
      ...
      // Then, write in the actual element
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    // MISSING:
    // Write the record. we always do this
    ...
    // Write the actual bucket. Only do this if there
    // are any links to write in the first place
    int bucketlen = NumElementsInLinkedList(ll);
    if (bucketlen > 0) {
