...
int main(int argc, char **argv) {
  ...
  // get all string arguments and store in vector
  list<string> index_files;
  for (int i = 1; i < argc; i++) {
      index_files.push_back(argv[i]);
  }

  hw3::QueryProcessor qp(index_files);
  while (1) {
      ...
      // parse string into a vector of words
      string buf;
      ...
      for (hw3::QueryProcessor::QueryResult result : results) {
